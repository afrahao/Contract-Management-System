...
public class UserDaoImpl implements UserDao {
	...
	public boolean ChangePassword(String name, String nPassword) throws AppException {
		...
		try {
			conn = DBUtil.getConnection();// Create database connection
			// Declare operation statement, "?" is a placeholder
			String sql = "update t_user set password = ? where name = ?";
			
			psmt = conn.prepareStatement(sql);// Pre-compiled sql
			// Set values for the placeholder 
			psmt.setString(2, name);
			psmt.setString(1, nPassword);
			...
			e.printStackTrace();
			throw new AppException("com.ruanko.dao.impl.UserDaoImpl.delete");
		} finally {
			DBUtil.closeStatement(psmt);//  Close database object pretreatment
			DBUtil.closeConnection(conn);// Close database connection object
		}
	...
	public boolean isSame(String name, String oPassword) throws AppException{
		...
		try {
			...
			psmt = conn.prepareStatement(sql);//  Pre-compiled sql
			psmt.setString(1, name);// Set values for the placeholder
			psmt.setString(2, oPassword);// Set values for the placeholder
			...
			e.printStackTrace();
			throw new AppException(
					"com.ruanko.dao.impl.UserDaoImpl.isSame");
		} finally {
			DBUtil.closeResultSet(rs);// Close database query result set
			DBUtil.closeStatement(psmt);//  Close database object pretreatment
			DBUtil.closeConnection(conn);// Close database connection object
	...
	public boolean isExistUser(String name) throws AppException {
		...
		try {
			...
			e.printStackTrace();
			throw new AppException(
					"com.ruanko.dao.impl.UserDaoImpl.isExistUser");
		} finally {
			DBUtil.closeResultSet(rs);// Close database query result set
			DBUtil.closeStatement(psmt);//  Close database object pretreatment
			DBUtil.closeConnection(conn);// Close database connection object
	...
	public boolean isExist(int id) throws AppException {
		...
		try {
			conn = DBUtil.getConnection();// Create database connection
			// Declare operation statement, query records based on user name, "?" is a placeholder
			String sql = "select id from t_user where id = ? and del = 0";

			psmt = conn.prepareStatement(sql);//  Pre-compiled sql
			psmt.setLong(1, id);// Set values for the placeholder 
