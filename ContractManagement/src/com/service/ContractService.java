...
public class ContractService {
	...
	public boolean draft(Contract contract) throws AppException {
		...
		try {
			...
			if (contractDao.add(contract)) {
				...
				logDao.writeLog("draft "+contract.getName(), contract.getUserId(), new Date());
			}
	...
	public List<ConBusiModel> getAssignList() throws AppException {
	...
	public Contract FindLatest(List<ConBusiModel> list) throws AppException, ParseException {
		...
		if(list.size() != 0)
		{
			for(int j = 0;j < list.size();j++)
			{
				...
				//get contract
				conLatest = list.get(j);
				latest = contractDao.getById(conLatest.getConId());
				//judge begin date validation
				if(!mydate.before(latest.getBeginTime()))
				{
					latest = null;
				}
			}
			...
			{
				conLatest = list.get(i);
				Contract ConLatest = contractDao.getById(conLatest.getConId());
				if(mydate.before(ConLatest.getBeginTime()))
				{
					//select the closest day
					if(ConLatest.getBeginTime().after(latest.getBeginTime()))
					{
						latest = ConLatest;
					}
				}
		...
		if(latest != null)
		{
			if(daysBetween(mydate,latest.getBeginTime())>10)
			{
				latest=null;
			}
		}
	...
	public List<ConBusiModel> getCountersignList(int userId) throws AppException {
		// Initialize  conList
		...
		try {
			if (conProcessDao.update(conProcess)) { // To do countersign contract, enter countersigne information
				...
				if (totalCount == 0) {
					...
					flag = conStateDao.add(conState);
					logDao.writeLog("countersign " + contractDao.getById(conProcess.getConId()).getName(), conProcess.getUserId(), new Date());
				}
			}
		} catch (AppException e) {
			e.printStackTrace();
	...
	public List<ConBusiModel> getFinalizeList(int userId) throws AppException {
		// Initialize conList
		List<ConBusiModel> conList = new ArrayList<ConBusiModel>();
		// Initialize conIds,for saving id set of contracts that to be finalized
		List<Integer> conIds = new ArrayList<Integer>();
	...
	public boolean finalize(Contract contract) throws AppException {
		...
		try {
			...
			if (contractDao.updateById(contract)) {
				...
				flag = conStateDao.add(conState);
				logDao.writeLog("finalize " + contract.getName(), contract.getUserId(), new Date());
			}
		} catch (AppException e) {
			e.printStackTrace();
			throw new AppException(
	...
	public List<CSignatureOpinion> showCountersignOpinion(int conId) throws AppException {
	...
	public List<ConBusiModel> getFinalizedContract(int userId) throws AppException {
		// Initialize conList
				...
				try {
					...
					e.printStackTrace();
					throw new AppException(
							"com.service.ContractService.getFinalizedContract");
				}
				// Return conList
				return conList;
	}
	
	public List<ConBusiModel> getApproveList(int userId) throws AppException {
		// Initialize conList
		List<ConBusiModel> conList = new ArrayList<ConBusiModel>();
		// Initialize conList for saving id set of contract to be approved
		List<Integer> conIds = new ArrayList<Integer>();
		...
		try {
			...
			if (conProcessDao.update(conProcess)) { // To approve contract,enter approval information 
				...
				if (tbApprovedCount == 0 && refusedCount == 0) {
					...
					// Save contract state information
					flag = conStateDao.add(conState);
					logDao.writeLog("approve " + contractDao.getById(conProcess.getConId()).getName(), conProcess.getUserId(), new Date());
				}
			}
		} catch (AppException e) {
			e.printStackTrace();
	...
	public List<ConBusiModel> getApprovedContract(int userId) throws AppException {
		// Initialize conList
				...
				conProcess.setType(Constant.PROCESS_APPROVE);
				// Set corresponding state of "PROCESS_SIGN" type  is "DONE"
				conProcess.setState(Constant.DONE);
				...
				try {
					...
					e.printStackTrace();
					throw new AppException(
							"com.service.ContractService.getApprovedContract");
				}
				// Return conList
				return conList;
	}
	...
	public List<ConBusiModel> getAssignedContract(int userId) throws AppException {
		...
		conProcess.setUserId(userId);
		// Set process's operation type to "PROCESS_SIGN"
		conProcess.setType(Constant.PROCESS_ASSIGN);
		// Set corresponding state of "PROCESS_SIGN" type  is "DONE"
		conProcess.setState(Constant.DONE);
		...
		try {
			 ...
			 */
			for (int conId : myConIds) {
				if (conStateDao.isExist(conId, Constant.STATE_ASSIGNED)) {
					conIds.add(conId);
				}
			}
	...
	public List<ConBusiModel> getSignList(int userId) throws AppException {
		// Initialize conList
		List<ConBusiModel> conList = new ArrayList<ConBusiModel>();
		// nitialize conIds for saving contract id set that to be signed
		List<Integer> conIds = new ArrayList<Integer>();
	...
	public boolean sign(ConProcess conProcess) throws AppException {
		...
		try {
			if (conProcessDao.update(conProcess)) {// Sign contract:update contract process information
				...
				// Save contract state information
				flag = conStateDao.add(conState);
				logDao.writeLog("sign " + contractDao.getById(conProcess.getConId()).getName(), conProcess.getUserId(), new Date());
			}
		} catch (AppException e) {
			e.printStackTrace();
			throw new AppException(
	...
	public boolean assign(ConProcess conProcess) throws AppException {
		...
		try {
			if (conProcessDao.update(conProcess)) {// Sign contract:update contract process information
				...
				conState.setConId(conProcess.getConId());
				// Set contract state type to "STATE_SIGNED"
				conState.setType(Constant.STATE_ASSIGNED);
				// Save contract state information
				flag = conStateDao.add(conState);
				logDao.writeLog("assign " + contractDao.getById(conProcess.getConId()).getName(), conProcess.getUserId(), new Date());
			}
		} catch (AppException e) {
			e.printStackTrace();
			throw new AppException(
					"com.service.ContractService.assign");
		}
		return flag;
	}
